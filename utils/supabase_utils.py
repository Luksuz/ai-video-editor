...
def increment_breakpoints_completed(video_id):
    ...
    try:
        ...
        get_response = (
            supabase.table("videos").select("breakpoints_completed").eq("id", video_id).execute()
        )
        ...
        if response.data and len(response.data) > 0:
            logger.info(
                f"Incremented breakpoints_completed for video {video_id} to {current_breakpoints_completed + 1}"
            )
...
def set_video_status(video_id, status):
    ...
    try:
        ...
        response = (
            supabase.table("videos")
            .update({"status": status, "updated_at": time.strftime("%Y-%m-%d %H:%M:%S")})
            .eq("id", video_id)
            .execute()
        )
