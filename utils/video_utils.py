...
def search_pexels_videos(query, per_page=10, api_key=None, used_urls=None, target_duration=None):
    ...
    if api_key is None:
        ...
        if not api_key:
            raise ValueError("No API key provided and PEXELS_API_KEY not found in environment variables")
...
def create_simple_video(output_path, duration, width=1920, height=1080):
    ...
    try:
        cmd = [
            ...
            '-force_key_frames', f'expr:gte(t,n_forced*1)',  # Force keyframe every second
