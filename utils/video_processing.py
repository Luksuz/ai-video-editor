...
import platform
import logging
from .video_utils import get_video_duration, get_audio_duration
...
def process_video_for_audio(input_path, output_path, target_duration, max_speed_change=1.5, target_width=1920, target_height=1080):
    ...
    try:
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            ...
            logger.info(f"Final video duration: {final_duration:.2f}s (target: {target_duration:.2f}s)")
            return True
        else:
...
def concatenate_videos_with_crossfade(video_paths, output_path, transition_duration=0.5):
    ...
    try:
        ...
        for batch_start in range(0, len(video_paths), batch_size):
            ...
            for i in range(len(batch_videos) - 1):
                ...
                if i == 0:
                    ...
                    filter_complex += f"[a{i-1}{i}][{i+1}:a]acrossfade=d={transition_duration}[a{i}{i+1}];"
            ...
            cmd.extend([
                ...
                '-map', f'[v{last_idx}{last_idx+1}]',
                '-map', f'[a{last_idx}{last_idx+1}]',
                '-c:v', video_codec,
                '-preset', preset,
                '-pix_fmt', 'yuv420p',
                '-c:a', 'aac',
        ...
        if len(intermediate_videos) > 1:
            ...
            subprocess.run(final_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=final_timeout)
