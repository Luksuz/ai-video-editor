...
class AudioProcessingRequest(BaseModel):
    ...
    output_dir: Optional[str] = Field("output_videos", description="Directory to save output videos")
...
class VideoResponseWithStorage(BaseModel):
    ...
    original_url: Optional[str] = Field("", description="URL to the original video in Supabase storage")
    preview_url: Optional[str] = Field("", description="URL to the preview video in Supabase storage")
    processing_time: float = Field(..., description="Total processing time in seconds")
    success: bool = Field(..., description="Whether the processing was successful")
    message: str = Field("", description="Additional information or error message")
