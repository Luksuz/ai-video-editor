...
@router.post("/process-and-store", response_model=VideoResponseWithStorage, status_code=status.HTTP_202_ACCEPTED)
async def process_audio_and_store(request: AudioProcessingRequest, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        return VideoResponseWithStorage(
            ...
            original_url=supabase_data.get("original_url", ""),  # Will be updated later in background task
            preview_url=supabase_data.get("preview_url", ""),    # Will be updated later in background task
            processing_time=processing_time,
            success=True,
            message=f"Processing started with ID: {supabase_data['video_id']}. Check status using this ID."
        )
