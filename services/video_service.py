...
from utils.video_processing import process_video_for_audio, combine_audio_video, concatenate_videos_with_crossfade
from utils.supabase_utils import upload_video_to_supabase, create_video_preview, save_video_metadata, save_video_breakpoints, increment_breakpoints_completed, set_video_status
...
async def process_audio_from_breakpoints_api(request_data, output_dir="output_videos", combine_videos=False, save_to_supabase=True, background_tasks=None):
    """
...
async def process_audio_in_background(request_data, output_dir, combine_videos, save_to_supabase, 
                                     temp_dir, full_audio_path, original_filename, breakpoints, video_id):
    """
...
async def process_audio_complete(temp_dir, full_audio_path, original_filename, breakpoints, video_id, 
                                output_dir, combine_videos, save_to_supabase):
    """
    ...
    try:
        ...
        for i in range(len(breakpoints) - 1):
            ...
            if os.path.exists(chunk_path) and os.path.getsize(chunk_path) > 0:
                ...
                logger.info(f"Created chunk {i+1}/{len(breakpoints)-1}: {start_time:.2f}s to {end_time:.2f}s (duration: {duration:.2f}s)")
            else:
        ...
        created_videos = await process_audio_chunks_local(audio_chunks, sentence_classifier, output_dir)
        ...
        if combine_videos and len(created_videos) > 1:
            ...
            if concatenate_videos_with_crossfade(created_videos, output_path, transition_duration=0.5):
        ...
        logger.info("="*50)
        logger.info(f"Total processing time: {overall_time:.2f} seconds ({overall_time/60:.2f} minutes)")
        logger.info(f"Number of chunks processed: {len(audio_chunks)}")
        logger.info(f"Average time per chunk: {timing_report.get('process_chunks', 0)/max(1, len(audio_chunks)):.2f} seconds")
        logger.info("-"*50)
...
async def process_audio_chunks_local(audio_chunks, sentence_classifier, output_dir="output_videos"):
    ...
    for i, chunk in enumerate(audio_chunks):
        ...
        try:
            ...
            if audio_duration <= 0:
                audio_duration = chunk['chunkDuration']  # Use the provided duration if ffprobe fails
            ...
            if not transcription:
                logger.warning(f"Failed to transcribe audio chunk {i+1}, using filename as fallback")
            ...
            result = await Runner.run(sentence_classifier, f"Find a relevant video for this audio: {transcription}")
            ...
            if video_data.get('videos') and len(video_data['videos']) > 0:
                ...
                for video_index, video in enumerate(video_data['videos']):
                    ...
                    logger.info(f"Video {video_index+1}: Duration {pexels_duration:.2f}s (diff: {duration_match:.2f}s)")
                    ...
                    if download_url and download_url not in used_video_urls:
                        ...
                        timing_report[f"download_video_{video_index}"] = time.time() - download_start
                        ...
                        if download_success:
                            ...
                            timing_report[f"check_validity_{video_index}"] = time.time() - validity_start
                            ...
                            if is_valid:
                                ...
                                timing_report[f"process_video_{video_index}"] = time.time() - processing_start
                                
                                if processing_success:
                                    # Combine audio and video
                                    combine_start = time.time()
                                    final_path = os.path.join(final_dir, f"final_{i:03d}.mp4")
                                    combine_success = combine_audio_video(video_path, audio_path, final_path)
                                    timing_report[f"combine_audio_video_{video_index}"] = time.time() - combine_start
        ...
        logger.info("="*50)
        logger.info(f"Audio duration: {audio_duration:.2f} seconds")
        logger.info(f"Total processing time: {chunk_total_time:.2f} seconds ({chunk_total_time/60:.2f} minutes)")
        logger.info("-"*50)
